...
def get_raw_lines(
    ...
    def sanitize_spans(line):
        ...
        for i in range(len(line) - 1, 0, -1):  # iterate back to front
            ...
            delta = s1["size"] * 0.1
            # Preserve baseline-shifted small spans and all style boundaries.
            # PyMuPDF's style recovery supplies script metadata and exact
            # decorator char_flags; joining across either would discard style
            # information before the Markdown serializer can consume it.
            smaller, larger = (
                (s0, s1) if s0["size"] <= s1["size"] else (s1, s0)
            )
            shifted_small_span = smaller["size"] < 0.85 * larger["size"] and (
                abs(smaller["origin"][1] - larger["origin"][1])
                > 0.1 * larger["size"]
            )
            # MuPDF's native underline flag can alternate at individual glyphs
            # when a decorator crosses only descenders or a table border. Keep
            # the established native normalization, but preserve exact
            # boundaries generated by PyMuPDF's guarded recovery API.
            recovered_boundary = s0.get("recovered_style") or s1.get(
                "recovered_style"
            )
            char_flags0 = (
                s0["char_flags"]
                if recovered_boundary
                else s0["char_flags"] & ~pymupdf.mupdf.FZ_STEXT_UNDERLINE
            )
            char_flags1 = (
                s1["char_flags"]
                if recovered_boundary
                else s1["char_flags"] & ~pymupdf.mupdf.FZ_STEXT_UNDERLINE
            )
            if shifted_small_span or s0["bbox"].x1 + delta < s1["bbox"].x0 or (
                s0["flags"],
                char_flags0,
                s0.get("script"),
                # s0["size"],
    ...
    # Keep recomputed-line identity available to the serializer. This lets it
    # distinguish a style boundary inside one source line from a visual gap or
    # a line transition without reconstructing layout a second time.
    for line_number, (_, line_spans) in enumerate(nlines):
        for span in line_spans:
            span["_reconstructed_line"] = line_number
